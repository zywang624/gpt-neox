...
class ParallelSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.lskv_bottleneck_dim % 2 != 0:
            raise ValueError(
                f"lskv_bottleneck_dim must be even when use_alpha_routing is on, "
                f"got {self.lskv_bottleneck_dim}"
            )
        self.lskv_bottleneck_dim_half = self.lskv_bottleneck_dim // 2
        ...
        self.down_up_proj_half = nn.Sequential(
            nn.Linear(neox_args.hidden_size, self.lskv_bottleneck_dim_half, bias=False),
            # nn.GELU(),
            nn.Linear(self.lskv_bottleneck_dim_half, neox_args.hidden_size, bias=False),
        )
        print(
    ...
    def forward(self, hidden_states, alpha, attention_mask, layer_past=None):

        # hidden_states: [sq, b, h]
        # alpha:         [sq, b, h]   (per-token scalar broadcast to hidden;
        #                              see AlphaRouterPipe docstring for why)
        ...
        lt_full = self.down_up_proj(hidden_states)
        lt_half = self.down_up_proj_half(hidden_states)
        # alpha is [s, b, h] with the same per-token scalar replicated across
        # the hidden dim, so we can use it directly as an elementwise mix
...
class ParallelTransformerLayer(nn.Module):
    ...
    def forward(self, x, alpha, attention_mask, layer_past=None):
        layer_past = layer_past if layer_past is not None else self.layer_past
        bias_dropout_fn = self._get_bias_dropout()
        # x:     [s, b, h]   (post _pre_transformer_block)
        # alpha: [s, b, 1]
        if self.gpt_j_residual:
            # pseudocode:
            ...
            attention_output, attention_bias = self.attention(
                x1, alpha, attention_mask, layer_past=layer_past
            )
            if self.use_cache:
            ...
            # x = x + attn(ln1(x))
            attention_output, attention_bias = self.attention(
                self.input_layernorm(x), alpha, attention_mask, layer_past=layer_past
            )
            if self.use_cache:
                attention_output, presents = attention_output
                self.layer_past = presents
...
class ParallelTransformerLayerPipe(ParallelTransformerLayer):
    """Extends ParallelTransformerLayer to forward (alpha, attention_mask) through the pipeline."""

    def forward(self, args):
        assert (
            len(args) == 3
        ), "ParallelTransformerLayerPipe expects 3 arguments - (hidden_states, alpha, attention_mask)"
        hidden_states, alpha, attention_mask = args
        # we are returning [hidden_states, alpha, mask]
        return super().forward(hidden_states, alpha, attention_mask), alpha, attention_mask
