...
class GPTNeoXAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # alpha: [bs, seq_len, 1] (per-token scalar) — required for DDIM
        alpha = kwargs.get("alpha", None)
        assert alpha is not None, (
            "DDIM attention requires `alpha` to be threaded through from the "
            "model forward via kwargs. Make sure GPTNeoXModel forward is the "
            "DDIM variant that computes and passes alpha."
        )
